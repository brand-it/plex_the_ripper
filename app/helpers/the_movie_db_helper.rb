...
module TheMovieDbHelper
  ...
  def link_to_movie_db_seasons(tv_id, season_number, text: 'go-to the movie DB')
    link_to text, "https://#{HOST}/tv/#{tv_id}/season/#{season_number}", target: :blank
  end

  def link_to_movie_db_tv(tv_id, text: 'go-to the movie DB')
    link_to text, "https://#{HOST}/tv/#{tv_id}", target: :blank
  end

  def link_to_movie_db_movie(movie_id, text: 'go-to the movie DB')
    link_to text, "https://#{HOST}/movie/#{movie_id}", target: :blank
  end

  def link_to_movie_db_episode(tv_id, season_number, episode_number, text: 'go-to the movie DB')
